...
const GoogleLogin: React.FC<GoogleLoginProps> = ({ onFailure }) => {
  const handleLogin = () => {
    try {
      if (!GOOGLE_CLIENT_ID) {
      ...
      if (onFailure) onFailure(error instanceof Error ? error : new Error('구글 로그인 실패'));
    }
  };
  ...
  return (
    <button 
      onClick={handleLogin}
      className="w-12 h-12 bg-white hover:bg-gray-50 border border-gray-200 rounded-full flex items-center justify-center transition-colors duration-200"
      type="button"
      aria-label="구글 로그인"
    >
